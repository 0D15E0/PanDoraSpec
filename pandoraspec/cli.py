...
app = typer.Typer(help="DORA Audit CLI - Verify Compliance of OpenAPI Specs")
console = Console()
...
def run_audit(
    ...
    """
    display_vendor = vendor or "Unknown Vendor"
    console.print(Panel(f"[bold blue]Starting DORA Audit for {display_vendor}[/bold blue]", border_style="blue"))
    ...
    try:
        ...
        # Drift
        drift_pass = sum(1 for r in results["drift_check"] if r.get("status") == "PASS")
        drift_fail = sum(1 for r in results["drift_check"] if r.get("status") != "PASS")
        ...
        # Resilience
        res_pass = sum(1 for r in results["resilience"] if r.get("status") == "PASS")
        res_fail = sum(1 for r in results["resilience"] if r.get("status") != "PASS")
        ...
        # Security
        sec_pass = sum(1 for r in results["security"] if r.get("status") == "PASS")
        sec_fail = sum(1 for r in results["security"] if r.get("status") != "PASS")
        ...
        console.print(
            Panel(
                ...
                f"📄 Report generated: [link={audit_result.report_path}]{audit_result.report_path}[/link]",
                border_style="green",
            )
        )
...
def main() -> None:
    """
    Entry point: dispatches to the Typer sub-command app (init, validate)
    or falls back to the legacy audit behaviour when a bare URL/path is given.
    """
